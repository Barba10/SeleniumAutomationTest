...
public class BinderPage extends TestBase {
	...
	public void editBinder() throws IOException, InterruptedException {
		TestUtil.click(editBinder, "Edit binder");
		TestUtil.click(outlineView, "Outline view");
		wait.until(ExpectedConditions.visibilityOf((addButton)));
		TestUtil.click(addButton, "Add button");
		wait.until(ExpectedConditions.visibilityOf((existingDocuments)));
		TestUtil.click(existingDocuments, "Existing documents");
		TestUtil.type(searchBar, "Search bar", "EAGLES Exercise 2 (v0.1)" + Keys.ENTER);
		wait.until(ExpectedConditions.visibilityOf((dragElement)));
		dragAndDropeSupporting();
		Thread.sleep(5000);
		searchBar.clear();
		TestUtil.type(searchBar, "Search bar", "BAS 455 37 H FR ReReg (v0.1)"+Keys.ENTER);
		dragAndDropeSupporting();
		Thread.sleep(5000);
		searchBar.clear();
		TestUtil.type(searchBar, "Search bar", "BAS 455 37 H Core S ReReg (v0.1)"+Keys.ENTER);
		dragAndDropeSupporting();
		TestUtil.click(closeExistingDocuments, "Close Button");
		TestUtil.click(doneEditing, "Done editing");
